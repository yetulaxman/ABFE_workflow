...
rule fep_run_complex_prod:
    ...
    params:
        ...
        gmx_template=gromacs_cont_script
    output:
        tpr=run_path+"/complex/fep/simulation/{state}/prod/prod.tpr",
      #  gro=run_path+"/complex/fep/simulation/{state}/prod/temp.gro",
      #  xvg=run_path+"/complex/fep/simulation/{state}/prod/temp.xvg"
    threads: num_sim_threads
    retries: num_retries
    shell:
        '''
            ...
            chmod +x ./job_prod.sh
            sed -i -e 's/gmx mdrun/# gmx mdrun/g' job_prod.sh
     #       touch temp.gro
     #       touch temp.xvg
